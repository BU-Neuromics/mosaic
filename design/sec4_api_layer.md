...
| OR filter composition | Medium | **Multi-value params implemented (issue #102)** — a repeated field param is an `in` filter, composed with other fields per `filter_mode`. The `?filter=` CEL expression form remains unimplemented; SQLite adapter would translate common patterns, complex expressions falling back to in-memory evaluation. |
...
| No relationship predicates yet | Filtering is typed and structured — the flat `filters:` list (`FilterOp`: `EQ`/`IN` v0.12.0 #102; `NEQ`/`GT`/`GTE`/`LT`/`LTE`/`CONTAINS`/`IS_NULL` ADR-0006 inc. 1) plus the generated `where: <Type>Filter` argument (per-slot operator objects + `and`/`or`/`not`, ADR-0006 inc. 2) — but reference edges cannot be filtered through yet: to-one nesting (M5a) and to-many `some`/`none` quantifiers (M5b) are the remaining ADR-0006 increments. Multivalued references stay walled off (`relatedTo` covers reverse lookups). |
