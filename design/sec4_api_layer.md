...
| Equality and `IN` filters only | `FilterInput` carries `op: FilterOp = EQ` with `EQ`/`IN` since v0.12.0 (issue #102), with AND/OR composition, mirroring `client.query`. Other comparison operators (`gt`/`lt`/`ne`/`contains`/…) raise a coded error rather than silently degrading to equality (issue #129). Richer typed predicates are a design question in their own right (see the decisions index). |
