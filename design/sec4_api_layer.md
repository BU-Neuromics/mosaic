...
| Flat filter list only | `FilterInput` carries `op: FilterOp` — `EQ`/`IN` (v0.12.0, issue #102) plus `NEQ`/`GT`/`GTE`/`LT`/`LTE`/`CONTAINS`/`IS_NULL` (ADR-0006 increment 1, issue #155) — with AND/OR composition, mirroring `client.query`. Each slot supports the operators its kind/range allows; anything else raises a coded error rather than silently degrading (issue #129). Nested boolean groups and relationship predicates arrive with the generated `where:` argument (ADR-0006 increments 2–4). |
