...
| GraphQL reference emission: raw scalar + resolved edge (dual) vs. edge-only | sec3, sec4 §4.7 | High | **🟢 Accepted 2026-07-17 ([ADR-0005](./decisions/ADR-0005-graphql-reference-emission-edge-only.md); [issue #131](https://github.com/BU-Neuromics/mosaic/issues/131)). ✅ Implemented in v0.12.0 ("BREAKING: edge-only GraphQL references").** A reference slot emits both a type-erased raw `*_id` scalar and a resolved edge (`_slot_spec`); the scalar drops the target class (misleads generic consumers like Aperture) and hard-codes a storage detail (would leak a future int surrogate key). Decision: **edge-only** — emit only the resolved relationship, remove the raw scalar, and adopt the *logical-identity boundary* invariant (physical/surrogate keys never cross the API). Clean break; references always resolve (collisions are build-time errors); multivalued cardinality is resolve-to-count by default, plus a cheap `count_relationship`/`<field>Count` primitive for the case that needs cardinality only (**✅ implemented**, [#132](https://github.com/BU-Neuromics/mosaic/issues/132)). Breaking change — sequenced ahead of the Aperture reference-rendering fixes. |
| Rich GraphQL filtering: typed per-class input objects vs. CEL on the wire | sec4 §4.3/§4.7 | High | **🟢 Accepted 2026-08-19 ([ADR-0006](./decisions/ADR-0006-graphql-typed-filter-contract.md); [issue #153](https://github.com/BU-Neuromics/mosaic/issues/153)). **All four increments implemented** (comparison operators; generated `<Type>Filter` + `where:`; M5a to-one relationship predicates; M5b some/none quantifiers) — the typed filter contract is complete (comparison operators on the flat path; generated `<Type>Filter` + `where:` with and/or/not trees).** Generated `<Type>Filter` input objects — per-slot operator objects (`eq`/`neq`/`in`/`gt`/`gte`/`lt`/`lte`/`contains`/`isNull` selected by slot kind/range) plus `and`/`or`/`not` — as a `where:` arg beside the flat `filters:`. Relationship predicates phase in: to-one nesting (correlated EXISTS on the FK column) before to-many `some`/`none` quantifiers against the relationships link table. **CEL explicitly not on the GraphQL wire** (opaque to introspection; stays for validator conditions) — supersedes/narrows the sec4 §4.3 CEL sketch for the GraphQL transport. `asOf` + relationship predicates rejected with a coded error until the temporal join is designed. Driven by Aperture cross-class queries (aperture `design/cross-class-query.md` §7; Aperture ADR-0035, Proposed). Implementation: OpenSpec `typed-filter-inputs` ([issue #155](https://github.com/BU-Neuromics/mosaic/issues/155)). |
| Aggregation & ordering on the list surface (counts, facets, min/max, order_by) | sec4 §4.7, sec6 | High | **🟢 Accepted 2026-08-19 ([ADR-0007](./decisions/ADR-0007-aggregation-and-ordering-surface.md); [issue #154](https://github.com/BU-Neuromics/mosaic/issues/154)).** Count-mode queries (cheap filtered `total`), `facetCounts(field, where)`, min/max per slot, and a generated per-type `order_by` enum — shipped together because they share the limit/offset/order pushdown into the storage `Query` on both adapters. Pinned rules: **aggregates see exactly what list queries see** (`is_available = 1`; the include-unavailable `entity_counts()` precedent is not the model), and provenance-computed `created_at`/`updated_at` are never column sorts. Fills Aperture portal requirement X1 (previously hippo#96, zero footprint here). Implementation: OpenSpec `aggregation-and-ordering` ([issue #156](https://github.com/BU-Neuromics/mosaic/issues/156)) — **implemented** (all increments: pushdown + count mode, facet counts + min/max, generated `<Type>OrderField` ordering; gate decisions honored — `orderBy`+`asOf` coded error, facet/range roots current-state only). |
| Postgres storage model: single JSONB `entities` table vs. generated per-class typed tables | sec3b | High | **🟡 Proposed ([ADR-0008](./decisions/ADR-0008-postgres-per-class-tables-shared-sql-core.md); [issue #162](https://github.com/BU-Neuromics/mosaic/issues/162)).** The divergence is vestigial: v0.6.0 (PTS-168–174) migrated SQLite to per-class typed tables; Postgres never got the equivalent migration (`PostgresDDLGenerator` exists, "not-yet-wired-into-CRUD"), and every query-surface feature has been double-priced by JSONB casts/absence-semantics since. Proposed: wire the DDL generator into CRUD with a one-time JSONB→per-class migration, and extract a shared SQL core (`column_expr` + dialect hooks) so find/filter/aggregation compile once. Sequenced **after** the current query-surface campaign (#155–#158; SQLite-first deployment). |
| MCP boundary makes an outbound call to a planning service — on what terms? | sec2, sec8 | High | **🟡 Proposed ([ADR-0010](./decisions/ADR-0010-mcp-outbound-delegation-to-a-planning-service.md); [issue #186](https://github.com/BU-Neuromics/mosaic/issues/186), [PR #199](https://github.com/BU-Neuromics/mosaic/pull/199)).** Aperture's chat must build a `QuerySpec` across turns; it has no backend (Aperture ADR-0032 rejected a control-plane service) and must not hold model credentials, so Mosaic relays to `mosaic-demo-small`'s Exon. Mosaic's **first egress HTTP call** — a deployment-shape change, not just a new tool. Terms: configured-or-absent registration; the planner is **untrusted** and its `QuerySpec` is re-validated in-process before any `proposal` is returned; strict response-shape checks; every failure a structured `error` turn, never an exception; client-visible errors name the *role* not the *address*; never executes; the wire contract (Decision 8) is not Mosaic's to change. Read-only posture and ADR-0009 item 5 unchanged. **Status Proposed** — implemented and verified end to end, but the terms were settled in PR review, not a design session. |
| Reverse-edge traversal in `QuerySpec` / `where:` (FK-backed reverse of a to-one reference) | sec3, sec4 §4.7 | High | **🟢 Accepted ([ADR-0011](./decisions/ADR-0011-inverse-slots-as-virtual-reverse-edges.md); [issue #204](https://github.com/BU-Neuromics/mosaic/issues/204)).** A `RelatedCondition.edge` can only name a slot the anchor holds, so `Donor → samples` cannot be expressed even though `Sample.donor` is a plain FK column; blocks `mosaic-demo-small`'s chat grounding. Checked on head: validator and compiler are manifest-driven and already pass a reverse edge once the schema declares one — only the storage adapters fail (they resolve every multivalued edge through the ADR-0002 link table). Decision: implement LinkML **`inverse`-declared slots as virtual reverse edges** — no column/link table/relationship rows; hydrated, filtered (one correlated `EXISTS` on the forward column), and counted through the forward FK; `inverse_of` exposed in the type model and capability manifest; writes carrying the slot ignored; reverse of a *multivalued* forward slot rejected at load. No auto-detection of hand-authored pairs (unsound). **Ratified 2026-09-22** ([#217](https://github.com/BU-Neuromics/mosaic/issues/217)) — implemented (`7fc300c`) and covered across core/GraphQL/MCP/both adapters, with an independent consumer-side check from Aperture ADR-0041. Two limits stated rather than implied: **no tag contains the merge** (latest `v0.13.0`, which certification pins — [#218](https://github.com/BU-Neuromics/mosaic/issues/218)), and no deployment LinkML declares an `inverse:` slot yet, so the consumer-side blocker moves from engineering to release + schema authoring rather than vanishing. |
| Per-actor accounting/observability across an egress hop | sec8 §8.6 | Medium | **🔴 Unspecified — surfaced by [ADR-0010](./decisions/ADR-0010-mcp-outbound-delegation-to-a-planning-service.md).** sec8 settles *inbound* auth (Bridge validates, injects `X-DataHelix-Actor`; Mosaic trusts and holds no keys; audit trail split Bridge/provenance, correlated by `request_id`) but has **no counterpart for Mosaic as a client to a downstream paid service** — no mention of egress, quota, or rate limiting, and it predates the MCP transport. So Bridge will know *who called Mosaic* while nothing knows *what Mosaic spent on whose behalf*. Intended shape: §8.6's two-log correlation extended one hop. Blocked on a Decision 8 amendment in `mosaic-demo-small` to carry an identity (ADR-0010 E2), since Mosaic must not add wire fields unilaterally; the actor is mechanically available at the tool today but deliberately dropped. Relatedly, **unauthenticated cost amplification** against a paid endpoint is a new exposure ADR-0009's "same trust boundary as REST" does not cover. Authn/authz itself remains [#54](https://github.com/BU-Neuromics/mosaic/issues/54) Part A. |
