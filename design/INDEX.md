...
| Whole-`upgrade()` multi-entity transaction atomicity | sec11 §11.8 | High | Open — does `client.load_context()` / per-write transaction guarantee full-upgrade rollback when `upgrade()` fails mid-way across multiple entity types? Or is rollback per-write only? Resolve in S3 before implementing the multi-hop chain. |
| Runtime `loader_depends_on` ordering | sec11 §11.8 | Medium | Open — does any runtime path (e.g., `hippo reference upgrade`) order loader invocations by `loader_depends_on` today? Assumption: no — annotation is documentation-only. If the runtime already orders, the S4 orchestrator can defer to it rather than re-implementing topological sort. Resolve in S3/S4. |
