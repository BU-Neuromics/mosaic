...
| Graph-level / query-spanning as-of reconstruction | sec6 §6.8, sec4 | High | **🟡 Designed — see [sec6 §6.8](./sec6_provenance.md) + [`decisions/ADR-0001`](./decisions/ADR-0001-graph-level-as-of-query.md) (sub-questions resolved). Pending ratification + OpenSpec implementation.** Generalizes per-entity `state_at` to a whole subgraph (entities + relationships + schema version) at a transaction-time `T`; relationship liveness reconstructed from `relationship_add`/`remove` provenance events; additive `as_of` on REST/GraphQL; decomposed into 5 increments. Required by Aperture data-story reproducibility (Aperture ADR-0023). |
