...
The TUI is a *consumer* of Hippo: every view talks to a ``TUIBackend`` and
never to ``HippoClient`` or HTTP directly. Both backends translate their
...
def derive_relationships(
    ...
    Hippo's graph-shaped API stores references as UUID-valued slots whose
...
def record_to_detail(
    ...
    Records have the shape returned by ``HippoClient.get()`` /
