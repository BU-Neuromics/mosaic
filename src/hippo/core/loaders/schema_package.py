...
This module is the *spine* of the SchemaPackage refactor (Hippo Migration
& Extension Design, Doc 2 §2A). It abstracts the reusable part of
:class:`~hippo.core.loaders.reference.ReferenceLoader` — *"contribute a
...
``SchemaPackage`` (genus)
    ...
    ``schema_fragment()`` (which must set ``default_prefix = name``; Hippo
...
Species (built incrementally across the sprint)
    - :class:`~hippo.core.loaders.reference.ReferenceLoader` — external
...
if TYPE_CHECKING:
    ...
    from hippo.core.loaders.reference import LoadResult
...
class SchemaPackage(ABC):
    ...
    name: str
    #: Human-readable one-liner surfaced by ``hippo`` listings.
    description: str

    # Optional Typer sub-app mounted under ``hippo reference <name> ...``
    ...
    def versions(self) -> list[str]:
        ...
        Slugs are opaque to Hippo (D2.14.C); format is package-defined.
    ...
    def schema_fragment(self) -> dict:
        ...
        Must declare ``default_prefix: <name>`` (D2.14.G, Rule 1). Hippo
    ...
    def provision(
        ...
        ``hippo migrate``), so they leave this as the no-op.
        """
        return None
...
class ExternalData(Protocol):
    ...
    Satisfied by :class:`~hippo.core.loaders.reference.ReferenceLoader`,
