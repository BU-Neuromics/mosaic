...
dependency (no ``typer``/``argparse``); the ``hippo reference`` CLI verbs
...
is the bridge between the ``requires:`` directive (:mod:`hippo.requires`) and
...
The CLI module ``hippo.cli.commands.reference`` re-exports the public names
here for backwards compatibility, so existing imports of
``hippo.cli.commands.reference.discover_schema_packages`` (etc.) keep working.
"""
...
from hippo.core.loaders.reference import ReferenceLoader, SchemaPackage
