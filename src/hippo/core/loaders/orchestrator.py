...
orchestrator that subsumes the per-loader, *unordered* ``hippo reference
upgrade`` path. It:
...
2. Drives the deployment through the target :class:`~hippo.core.loaders.bundle.Bundle`'s
...
3. Wraps the whole chain in a single staged commit-or-rollback scope
   (:meth:`HippoClient.staged_transaction`). After all hops it runs the
...
(:mod:`hippo.core.loaders.exposure`) *warns* about the same footprint
...
def topological_sort(
    ...
    Raises :class:`~hippo.core.exceptions.OrchestrationError` if the graph
...
def run_end_to_end_gate(
    ...
    *,
    ...
    :class:`~hippo.core.exceptions.MigrationGateError` and rolling the whole
    ...
    with tempfile.TemporaryDirectory(prefix="hippo-e2e-gate-") as tmp:
...
class OrchestrationResult:
    ...
    after a clean return (matching hippo's existing evolve-then-record
...
def migrate_to_bundle(
    ...
    *,
    ...
    The whole chain runs inside one :meth:`HippoClient.staged_transaction`:
    ...
    Raises :class:`~hippo.core.exceptions.OrchestrationError` (bad graph /
