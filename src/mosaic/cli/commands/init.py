"""Init command implementation for Mosaic CLI."""
...
from mosaic.cli.templates import (
    get_template,
...
class InitCommand:
    """Handles project initialization for Mosaic."""
    ...
    def _generate_files(self) -> None:
        ...
        typer.echo(f"\nMosaic project initialized at {self.target_path}")
        typer.echo(f"Template: {template.name}")
        typer.echo(f"Storage: {self.storage}")
        typer.echo("Run 'mosaic serve' to start the server")
...
def run_init(
    ...
    """Entry point for mosaic init command."""
