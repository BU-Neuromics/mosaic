...
* the base migration's **structural delta** — the classes and slots that
  ...
  the programmatic form of ``mosaic schema-diff <old> <new>``);
...
*or* the end-to-end gate (:func:`~mosaic.core.loaders.orchestrator.run_end_to_end_gate`)
...
def compute_write_set(
    ...
    *removed*, and in both but with a differing definition are *changed*.
    This is the programmatic equivalent of ``mosaic schema-diff`` over the
