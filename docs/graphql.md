...
Hippo can serve an autogenerated GraphQL API alongside the REST API. The GraphQL
...
delegates to the same `HippoClient` the SDK, REST, and TUI use, against the same
storage.
...
```bash
pip install 'hippo[graphql]'
```

(It is also included in `hippo[all]`.)

## Serving

```bash
hippo serve --graphql                 # auto-detect config in the cwd
hippo serve --graphql --config hippo.yaml
hippo serve --graphql --graphql-max-depth 6   # tighten the query-depth limit
```
...
```python
from hippo.core.factory import create_client_from_config, load_config_autodetect
from hippo.serve import create_default_app

config = load_config_autodetect("hippo.yaml")
...
For each concrete entity class in your schema (say `Sample`), Hippo generates:
...
The exposed class set is decided by Hippo's shared type model
(`hippo.core.schema_typing`) — the same model behind the typed Python SDK — so
...
There are no delete mutations. Hippo never hard-deletes; use availability
transitions:
...
### Schema introspection (the Hippo kind)

Beyond GraphQL's own `__schema`, the API exposes Hippo's domain type model —
