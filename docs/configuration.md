...
Complete reference for configuring Mosaic, the LinkML runtime for the BASS platform (the platform's structured domain graph; metadata tracking is one application).

## Overview

Mosaic uses two primary configuration files:
...
Configuration is loaded via `load_mosaic_config()`, and schemas are loaded via `SchemaParser` or `load_schema()`.

---

## Config file discovery (`mosaic.yaml`)

When no explicit `--config` is given, Mosaic scans the working directory in
this order: `config.json`, `mosaic.yaml`, `mosaic.yml`, then the legacy
`hippo.yaml` / `hippo.yml` (pre-rename spellings, ADR-0004 — still honored
with a `DeprecationWarning`). When both `mosaic.yaml` and `hippo.yaml` exist,
`mosaic.yaml` wins silently. `mosaic init` writes `mosaic.yaml`.

## Environment variables

Runtime environment variables use the `MOSAIC_` prefix (`MOSAIC_CACHE_DIR`,
`MOSAIC_RECIPE_CACHE`, `MOSAIC_TUI_TOKEN`, `MOSAIC_DATABASE_URL`). The legacy
`HIPPO_` spellings are honored as a fallback for the deprecation window — one
...
| `database_url` | `str` | `None` | Connection string for the database. For SQLite: `mosaic.db` or absolute path |
...
`schema_path` is **required** -- `MosaicConfig` will not instantiate without it.
...
### Template config.json (from `mosaic init`)

The `basic` template generated by `mosaic init` produces:
...
### Mosaic-Specific Annotations

Mosaic extends standard LinkML with storage annotations. These are expressed under the `annotations` key on attributes:
...
When the `range` is a class, Mosaic treats the attribute as an entity reference. This is used for `MosaicClient.schema_references()` and Cappella's collection resolver.
...
```python
from mosaic.config.loader import load_mosaic_config, load_schema

# Load main configuration
config = load_mosaic_config("config.json")

# Load a schema file
schema = load_schema("schemas/donor.yaml")

# Load all schemas from a directory
from mosaic.config.loader import SchemaParser
