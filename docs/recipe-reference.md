...
Complete field and command reference for the Hippo recipe system. For authoring walkthroughs see [Writing a Recipe](writing-a-recipe.md); for install and lockfile walkthroughs see [Installing Recipes](installing-recipes.md).
...
| `hippo_version` | yes | string | PEP 440 version specifier (e.g. `">=0.3,<1.0"`). Import fails with `RecipeVersionIncompatibleError` when the running Hippo version is excluded by this specifier. |
| `description` | no | string | Human-readable, one paragraph. |
| `license` | no | string | SPDX identifier (e.g. `CC-BY-4.0`). |
| `source` | no | string | Author-declared canonical origin URI (e.g. a Zenodo DOI URL). Metadata only — Hippo never fetches from this field. |
...
| `reference_loaders` | List of `name==version` pin strings. Treated as preconditions only — Hippo checks each is installed at the declared version; it does not install them automatically (see §10.4.5). |
...
| `source` | The exact URI Hippo fetched from (post-resolution). |
...
This registry is the source of truth for `hippo recipe list` and the input to `hippo recipe export-lockfile`.
...
`hippo recipe inspect` prints the computed digest so authors can verify it before publishing.

---

## CLI commands

### `hippo recipe inspect`

Parse, validate, and digest a recipe without writing any state.

```bash
hippo recipe inspect <source> [--show-elements]
```
...
Install a recipe and all its dependencies into the current Hippo instance.

```bash
hippo recipe import <source> [--digest <hash>] [--dry-run] [--db-path PATH] [--schema-dir DIR]
```
...
| `--db-path PATH` | SQLite database path (default: `data/hippo.db`). |
...
```bash
hippo recipe export --out PATH [--parent <installed-id>] [--db-path PATH] [--schema-dir DIR]
```

| Option | Description |
|---|---|
| `--out PATH` | Output directory. Writes `recipe.yaml` and `schema.yaml` here; fails if either file already exists. |
| `--parent <installed-id>` | `id` of an installed recipe to declare as the lineage parent. Until `hippo recipe list` ships (Phase 4), find installed ids in `hippo_meta.installed_recipes`. |
| `--db-path PATH` | SQLite database path (default: `data/hippo.db`). |
| `--schema-dir DIR` | Schema directory (default: `schemas/`). |

**Selectivity:** only classes and slots whose `provided_by` annotation is absent or does not start with `recipe.` or `loader.`, and whose containing schema is not a Hippo framework schema (`hippo_core`, `recipe_manifest`), are exported. Content contributed by installed recipes or Reference Loaders is excluded.
...
```bash
hippo recipe extend <installed-id> --out PATH
```

| Argument / Option | Description |
|---|---|
| `<installed-id>` | `id` of the installed recipe to extend (from `hippo recipe list`). |
...
### `hippo recipe export-lockfile` *(Phase 4)*
...
```bash
hippo recipe export-lockfile [--out PATH] [--db-path PATH]
```
...
### `hippo recipe install-from-lockfile` *(Phase 4)*
...
```bash
hippo recipe install-from-lockfile <file> [--db-path PATH]
```
...
|---|---|---|
| `HIPPO_RECIPE_CACHE` | `~/.hippo/recipe-cache/` | Override the cache directory used by `HttpsResolver` for fetched recipes. Useful for shared NFS mounts or CI cache volumes. |
...
| `RecipeVersionIncompatibleError` | The running Hippo version is excluded by the recipe's `hippo_version` specifier. |
