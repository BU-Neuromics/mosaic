...
An appendix document (`appendix_a_example_schema_omics.md`) SHALL exist in `design/` as a clearly labeled example deployment schema. It SHALL contain entity type definitions, field tables, relationship declarations in LinkML format, and an entity relationship diagram for an omics research deployment.
...
The appendix SHALL include all relationship declarations in LinkML format, covering: donated (Subject→Sample), derived_from (Sample→Sample, self-referential), generated (Sample→Datafile), input_to (Datafile→WorkflowRun), output_of (Datafile→WorkflowRun), instance_of (WorkflowRun→Workflow), and contains (Dataset→Datafile).
...
- **THEN** every relationship in the diagram corresponds to a declared relationship in the schema section, and vice versa
