...
## Acceptance Criteria
- Given a user has a valid LinkML schema file, when they run 'hippo validate-schema', then the CLI should validate the schema structure and output a success message to stdout
- Given a user has a schema file with syntax errors, when they run 'hippo validate-schema', then the CLI should display a clear error message indicating the location and type of syntax error in the schema file
